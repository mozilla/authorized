...
ifeq ($(buildid),)
  ...
  $(warning Install hg or set BUILDID the checkout id)
  $(error)
endif
...
build: setup
	...
	test -d $(stage_dir)/components || $(SLINK) $(TOPSRCDIR)/components $(stage_dir)/components
	

xpi_name := authorized-$(version)-$(xpi_type).xpi
xpi_files := chrome.manifest content install.rdf locale modules skin components
